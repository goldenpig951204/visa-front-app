...
const GlobalVisaPriceList = ({ visaTypes }) => {
  ...
  useEffect(() => {
    const getPrices = async () => {
      let { data } = await Http.get("crms/visa-prices", {
        params: {
          search: search
        }
      });
  ...
  const onSave = async (price) => {
    let { data } = await Http.post("crms/visa-prices", price);
    if (data.status) {
  ...
  const editPrice = async (id) => {
    setPriceId(id);
    let { data } = await Http.get(`crms/visa-prices/${id}`)
    editReset({
  ...
  const onUpdate = async (price) => {
    let { data } = await Http.put(`crms/visa-prices/${priceId}`, price);
    if (data.status) {
  ...
  const onDelete = async (id) => {
    let { data } = await Http.delete(`crms/visa-prices/${id}`);
    if (data.status) {
      setIsGetData(!isGetData);
      toast.success(data.msg);
    } else {
...
export default GlobalVisaPriceList;
